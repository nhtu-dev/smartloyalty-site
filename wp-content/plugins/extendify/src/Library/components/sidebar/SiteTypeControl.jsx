...
export const SiteTypeControl = () => {
	...
	return (
		<PanelBody
			title={
				hasSiteType
					? sprintf(__('Site Type: %s', 'extendify'), siteType.name)
			...
			<PanelRow className="m-0 w-full p-4 border border-gray-300 overflow-y-auto max-h-half -mt-1.5 rounded-b flex flex-col gap-2">
				<SearchControl
					ref={searchRef}
					className="w-full"
					label={__('Search for your business type', 'extendify')}
					placeholder={__('Search for your business type', 'extendify')}
					value={search}
					onChange={setSearch}
				/>
				{searching && (
					<div className="flex w-full justify-center p-2">
						<span className="sr-only">{__('Fetching...', 'extendify')}</span>
						<Spinner />
					</div>
				)}
