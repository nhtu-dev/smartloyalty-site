...
import { waitFor200Response } from '@launch/lib/util';
...
export const CreatingSite = () => {
	...
	const doEverything = useCallback(async () => {
		...
		try {
			...
			if (activePlugins?.filter((p) => p.includes('woocommerce'))?.length) {
			...
			if (
				activePlugins?.filter((p) => p.includes('the-events-calendar'))?.length
			) {
				const eventsPage = {
					slug: 'events',
	...
	return (
		<Transition
			...
			className="bg-banner-main text-banner-text py-12 px-10 md:h-screen flex flex-col justify-between md:max-w-md shrink-0">
