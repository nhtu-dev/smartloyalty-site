...
function wpum_get_account_page_tabs() {
	...
	$tabs = array(
		'settings' => array(
			'name'     => esc_html__( 'Cài đặt', 'wp-user-manager' ),
			'priority' => 0,
		),
		'password' => array(
			'name'     => esc_html__( 'Mật khẩu', 'wp-user-manager' ),
			'priority' => 800,
		),
		'view'     => array(
			'name'     => esc_html__( 'Xem hồ sơ', 'wp-user-manager' ),
			'priority' => 900,
		),
		'logout'   => array(
			'name'     => esc_html__( 'Đăng xuất', 'wp-user-manager' ),
			'priority' => 999,
		),
	);
